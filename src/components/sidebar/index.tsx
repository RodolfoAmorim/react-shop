...
export function Sidebar() {
  ...
  return (
    <aside className='xl:w-80 xl:bg-slate-900 xl:pt-12 xl:max-h-screen'>
      <span className='hidden xl:block px-10'>
      ...
      <nav
        ...
        } fixed w-11/12 rounded-r-3xl p-8 shadow-xl bg-gradient-to-bl from-violet-500 to-violet-800 top-6 bottom-6 flex flex-col items-start justify-center transition-all duration-500 sm:overflow-auto sm:justify-start sm:w-4/6 md:top-10 md:bottom-10 xl:bg-transparent xl:relative xl:top-auto xl:left-auto xl:bottom-auto xl:from-slate-900 xl:to-slate-900 xl:rounded-none xl:w-full xl:visible xl:opacity-100 xl:translate-x-0 xl:pt-0`}
      >
        <button
          className='w-10 h-10 flex items-center justify-center bg-violet-50/30 text-violet-50 rounded-lg absolute top-8 right-8 xl:hidden'
          onClick={handleOpenSidebar}
        >
          <X strokeWidth={1.25} />
        </button>
        ...
        <div className='w-full flex items-center justify-start gap-4 xl:hidden'>
          <img
            ...
            className='w-12 rounded-full outline outline-4 outline-violet-300/50'
          />
          <p className='text-lg leading-relaxed tracking-wider font-normal text-violet-50/80'>
            Login
          </p>
        </div>

        <div className='scroll flex-1 w-full overflow-auto flex flex-col items-start justify-start gap-10 px-4 pb-8 mt-12 sm:flex-none md:flex-1 md:pb-0 xl:flex-1 xl:overflow-auto'>
          <SidebarGroup>
